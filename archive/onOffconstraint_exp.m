...
%                                       [ 0, -0.15, .65]];                 % deg/s *up is down and down is up
    depth_structure                 = 1;                                    % 0 = random cloud, 1 = ground plane, 2 = natural scene 
    if depth_structure              ==1
        gaze_angle                  = 60;                                   %deg from staring straight at ground
    ...
    %-----Array Settings
    z0                              = 5;                                    % fixation distance
    middle_dist                     = 5; 
    if depth_structure == 1
        z0 = height./cos(deg2rad(gaze_angle));
        middle_dist = z0;
    end                                  %meters
    cloud_dist                      = [middle_dist*.95, middle_dist*1.05];  % m range of distances   
% cloud_dist                      = [middle_dist*.85, middle_dist*1.15];  % m range of distances 
    ...
    for ii = 1:size(translation,1)
        if depth_structure == 1
            ...
            depth_limits                    = [z_dev-10*bumpiness, z_dev];       %limits of depths to test, first is original deviation distance
    ...
    for trial = 1:total_trials
        ...
        if test
            perm(trial) = 3;
            cond(1).steps = repmat(cond(1).steps(:,1),[1,length(cond(1).steps)]); %if don't want deviation
        end
