%% full flow experiment, staircasing with linear changes along the constraint and rotated axes
% updated HL 11/23/2020
clear all;
close all;
clc;
...
%vvv vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    %-----Subject Settings------------------ 
    ...
    n_trials                        = 20+.2*20;                             % Number of trials n+.2*n so +10% hard and +10% easy trials
    n_anchors                       = n_trials/1.2*.2;
    translation                     = [[ 0, 0.15, .65]];             
%                                       [ 0, -0.15, .65]];                 % deg/s *up is down and down is up
    ...
    for j=1:size(translation,1)
        for l = 1:length(theta)
            for k=1:n_staircases
                ...
                cond(n_conditions).anchors = []; %indices of interspersed hard/easy trials
                
                cond(n_conditions).step_idx = 1; % difficulty level (which velocity to choose)
                cond(n_conditions).step_history = NaN(1, n_trials); % to display staircase
                ...
                cond(n_conditions).z0 = z0; %save fixation depth
    ...
    for c = 1:length(cond)
        ...
        if cond(c).rotation == theta(end-1) || cond(c).rotation == theta(end) % get steps along constraint
            end_vel = calculate_cloud_flow(depth_limits(2), [devPos(1); devPos(2)], cond(c).translate, view_dist, z0);
            %         end_vel = [0; -2.5125];
            closer_vel = calculate_cloud_flow(depth_limits(1), [devPos(1); devPos(2)], cond(c).translate, view_dist, z0);
        ...
        idxanchor = randsample(10:n_trials, n_anchors);
    ...
    for trial = 1:total_trials
        ...
        if sum(cond(perm(trial)).count == cond(perm(trial)).anchors)
            step = cond(perm(trial)).step_history(cond(perm(trial)).count);
        else
            step = cond(perm(trial)).step_idx; %otherwise continue with the staircase
        end
        
        if cond(perm(trial)).LR(cond(perm(trial)).count) %if it's on the right
            idx = dev1;
            velocity_field(:,idx) = cond(perm(trial)).steps(:,step);
        else % if it's on the left
            idx = dev0;
            velocity_field(:,idx) = [-cond(perm(trial)).steps(1,step); cond(perm(trial)).steps(2,step)];
...
%            Provides feedback during training phase
    ...
    cond(perm(trial)).dev_history(:,cond(perm(trial)).count) = cond(perm(trial)).steps(:,step);
    cond(perm(trial)).step_history(cond(perm(trial)).count) = step;
...
%     
    if sum(cond(perm(trial)).count == cond(perm(trial)).anchors) % if an anchor trial just report correct/incorrect, don't change step_idx
        if cond(perm(trial)).LR(cond(perm(trial)).count) == resp
            cond(perm(trial)).resp_history(cond(perm(trial)).count) = 1;
        else
        end
    else %otherwise carry on with staircasing
        %1 up 2 down staircase proceducre
        if cond(perm(trial)).n_flips < 1 % if they haven't flipped yet
            % if they're correct
            if cond(perm(trial)).LR(cond(perm(trial)).count) == resp
