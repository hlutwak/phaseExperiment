%% full flow experiment, natural scene
% updated HL 06/23/2021
% clear all;
% close all;
% clc;
...
%vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
   %-----Subject Settings------------------ 
    ...
    view_dist                       = .5;                                  % m .57; psychophysics room: .35 .50
    screensize                      = [.697 .392];                          % m psychophysics room: [.405 .298], home: [.495 .312] [.697 .392]
    pixels                          = [1900 1200]*2;                          % psychophysics room: [1600 1200], home: [1920 1200]
    ...
    translation                     = [0, 0, 1.5];                            % m/s
%                                       [ 0, -0.15, .65]];                  % deg/s *up is down and down is up
    weberFrac                       = 1;                                    % magnitude deviation vector deg/s
...
%         theta             = [0 180];
    ...
    switch depth_structure
        ...
        case 1
            ...
            ds.bumpiness                   = 0.001;                               % random noise in ground plane
            ds.wall                        = 0;                                 % ground plane + wall starting in upper visual field
            ds.planeParallax               = 1;
        case 2
            img_label = 'lRange025.mat';
            load (img_label)
        ...
        case 3
            viewingdepths = [.05,200];
            ppcm = [39,-39];
            xyrat = pixels(1)/pixels(2);
            nframes = frame_rate*stimulus_duration;
            dim = [100,0,nframes+100]; %dimensions for ground plane
            xvals = linspace(-25,25,300);
            zvals = linspace(0, 200,200); 
...
%             observerTrajectory = repmat(translation/frame_rate, [nframes, 1]); %trajectory over one frame
            observerTrajectory = repmat(translation, [nframes, 1]); %trajectory over one frame
            
            thetadots = atan(observerTrajectory(:,2)./observerTrajectory(:,3))*.01;
            ...
            for ii=1:nframes
                ...
                dots = dots - position;
                observerRotation = [1, 0, 0; 0, cos(thetadots(ii)), -sin(thetadots(ii)); 0, sin(thetadots(ii)), cos(thetadots(ii))];
                dots = (observerRotation*dots')';
                depths(:,ii) = dots(:,3); % in eye centered coordinates now
                %     X(:,ii) = view_dist*(dots(:,1)-pos(1))./(dots(:,3)-pos(3));
                %     Y(:,ii) = view_dist*(dots(:,2)-pos(2))./(dots(:,3)-pos(3));
                
                X(:,ii) = 100*view_dist*(dots(:,1))./(dots(:,3));
                Y(:,ii) = -100*view_dist*(dots(:,2))./(dots(:,3));
    ...
    dot_density                     = 1.5;                                  % 1/dot_density = average degrees between apertures measured from the centers
...
%     devPos                          = [randi(view_window(1)/2);randi((view_window(2)-1)/2)];        
    devPos                          = [5; 3]; % position of deviation (y-axis flipped when displaying)
    cluster_window                  = [.5 .5]; % size of cluster window in visual angle
    ...
    for c = 1:length(cond)
        ...
        switch depth_structure
            ...
            case 2 % if from natural depth map
                ...
                %isolate tree
                tree = find(range_distance<range_distance(devPosPix(2), devPosPix(1))+1 & range_distance(devPosPix(2), devPosPix(1))-1);
%                 range_distance(tree) = 300;
            case 3
...
%         start_vel = end_vel;
        ...
        % velocity range
        velocity_range(:,1) = calculate_cloud_flow(depth_limits(1), devPos, cond(c).translate, view_dist, z0);
        velocity_range(:,2) = calculate_cloud_flow(depth_limits(2), devPos, cond(c).translate, view_dist, z0);
        ...
        cond(c).step_history(idxanchor(:,n_anchors/2+1:end)) = stairFrac;
    end
    
    
    %----Start the experiment-----
    
    Screen('DrawText',w,'A velocity deviation will occur on the left or the right',fix_hor-200, fix_ver-30,250);
...
for b = 1:n_blocks
    ...
    for trial = 1:trials_per_block
        ...
        switch depth_structure
            ...
            case 1
                [dots_m, dots_deg, dev0, dev1] = make_dot_plane(dot_density, jitter, ds.bumpiness,ds.height, ds.gaze_angle, ds.wall, view_window, text_diam, exclude, devPos);
            ...
            case 3
                [dots_m, dots_deg, dev0, dev1] = make_dot_cloud(dot_density, jitter, cloud_dist, view_window, text_diam, exclude, devPos);
                nearestidx = dsearchn([X(:,1) Y(:,1)],dots_deg');
                distances = vecnorm(dots_deg- [X(nearestidx,1)'; Y(nearestidx,1)']);
                nothingidx = find(distances>.5);
