...
%                                                                         % deg/s *up is down and down is up
    weberFrac                       = 2;                                    % magnitude deviation vector deg/s
    ...
    theta                           = [0 45 90 135 180 225 270 315];        % range directions to test around base velocity, last two correspond to on constraint
%      theta                           = [0 90];
...
%                 I(:,ii) = I(:,ii) & abs(X(:,ii)*ppcm(1))<pixels(1)/2 & abs(Y(:,ii)*ppcm(2))<pixels(2)/2;
    ...
    gabor_sf                        = 4;                                    % c/deg   original settings: 4
...
%     cluster_window                  = [1.5/dot_density 1.5/dot_density]; % size of cluster window in visual angle
    ...
    ITI                             = .25;                                    % Intertrial Inverval, Seconds
...
%         start_vel = end_vel;
    ...
    % get depth constraint steps
    % calculate velocity for closer and further depth
    closer_vel = calculate_cloud_flow(depth_limits(:,1), [devPos cluster], cond(1).translate, view_dist, z0);
    ...
    for d = 1:nDevs
        steps = [linspace(0, segment(1,d), stairFrac); linspace(0, segment(2,d), stairFrac)]; %amount to move along segment
        depth_steps_closer = start_vel(:,d)+steps;
        
        % rotate by theta
        depth_steps_farther = get_theta_off_constraint(depth_steps_closer, end_vel, 180);
...
for b = 1:n_blocks
    ...
    for trial = 1:trials_per_block
        ...
        if cond(perm(trial)+(b-1)*n_conditions).LR(cond(perm(trial)+(b-1)*n_conditions).count)
            idx = [dev1 cluster1];
            idx_depth = [dev0 cluster0];
            
            velocity_field(:,idx) = cond(perm(trial)+(b-1)*n_conditions).steps(:,step);
%             if  mod(b,2) % switch off if testing closer or farther for different blocks, block 1&3 closer, 1&4 farther
%                 velocity_field(:,idx_depth) = [-depth_steps_closer(1,step); depth_steps_closer(2,step)];
%             else
%                 velocity_field(:,idx_depth) = [-depth_steps_farther(1,step); depth_steps_farther(2,step)];
%             end
            velocity_field(:,idx_depth) = [-depth_steps_closer(1,step); depth_steps_closer(2,step)];
       
        else % if it's on the left
            idx = [dev0 cluster0];
            idx_depth =[dev1 cluster1];
            
            velocity_field(:,idx) = [-cond(perm(trial)+(b-1)*n_conditions).steps(1,step); cond(perm(trial)+(b-1)*n_conditions).steps(2,step)];
%             if  mod(b,2) % switch off if testing closer or farther for different blocks, block 1&3 closer, 1&4 farther
%                 velocity_field(:,idx_depth) = depth_steps_closer(:,step);
%             else
%                 velocity_field(:,idx_depth) = depth_steps_farther(:,step);
%             end
            velocity_field(:,idx_depth) = depth_steps_closer(:,step);
